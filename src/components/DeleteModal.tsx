...
const DeleteModal = ({ deleteItem, item, closeModal }: DeleteModalProps) => {
  // const { deleteModalDispatch } = useContext(DeleteModalContext);
  ...
  return (
    <div className='fixed top-0 left-0 flex justify-center items-center w-full h-full bg-slate-400/75'>
      <div className='max-w-xs p-6 flex flex-col gap-4 bg-slate-100 shadow-[3px_3px_0_0] shadow-zinc-800 z-50'>
        {/* <div onClick={() => deleteModalDispatch({ type: 'HIDE' })}> */}
        <div onClick={closeModal}>
          <XCircleIcon className='w-6 h-6 ml-auto cursor-pointer ' data-testid='close-icon'/>
        </div>
        ...
        <div className='flex justify-between'>
          ...
          <button
            // onClick={() => deleteModalDispatch({ type: 'HIDE' })}
            onClick={closeModal}
